        ...
        <div className="h-screen flex p-5 justify-center bg-[#11001c]">
            <div className="w-125 h-[760px] bg-[#290025] p-4 rounded-2xl shadow-2xl flex flex-col border border-[#4F0147] ">
                <div className="flex justify-around ">
                    ...
                    <div>
                        <button className="bg-red-500 p-2 w-25 rounded-lg">Leave</button>
                    </div>
                </div>
                <div className="flex-1 p-4 overflow-y-auto space-y-3 bg-[#35012C]">
                    {Chats.map((chat, index) => (
                        <div key={index} className={`max-w-[75%] p-3 rounded-xl text-sm ${chat.username === UserName ? "ml-auto bg-[#3A015C]  text-white rounded-br-md" : "bg-[#11001C] text-gray-200 border border-[#4F0147] rounded-bl-md"}`}>
                            <strong className="block text-xs mb-1 opacity-70 ">{chat.username}</strong>
                            {chat.message}
                        </div>
                    ))}
                </div>
                <div className="p-3 border-t border-[#4F0147] flex gap-2 bg-[#290025]">
                    ...
                    <button onClick={HandleMsg} className="bg-[#3A015C] text-white px-4 rounded-xl hover:bg-[#4F0147] transition"><Send /></button>
                </div>
            </div>
        </div>
    )
